...
export default function DonorDashboard() {
  ...
  // Status change handler (calls backend)
    ...
    <div className="space-y-8 max-w-7xl mx-auto px-1">
      ...
      {recentRequests.length > 0 && (
        <div>
          ...
          {/* Mobile Cards (same as before, using _id) */}
          <div className="md:hidden space-y-4">
            {recentRequests.map((req) => (
              ...
              >
                {/* ... (keep existing mobile design, just replace req.id with req._id) ... */}
              </div>
            ))}
          </div>
        </div>
      ...
      {/* If no requests: section is hidden */}
